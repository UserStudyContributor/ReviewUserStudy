...
public class MainTable extends JTable {
	...
	public TableCellRenderer getCellRenderer(int row, int column) {
		...
		if (score < -1) {
			...
			if (status == MainTable.REQUIRED) {
				if (isRecentPublication(row)) {
					renderer = MainTable.recentPublicationsRender;
				} else {
	...
	private boolean isRecentPublication (int row) {
		try {

			BibtexEntry be = sortedForGrouping.get(row);
			String year = be.getField("year");
	...
	veryGrayedOutRenderer, recentPublicationsRender;
	...
	private static final String recentPublicationColor = "200:200:200";

	public static void updateRenderers() {

		String[] rgb = MainTable.recentPublicationColor.split(":");

		int[] irgb = new int [3];
		for (int i = 0 ; i < 3; i++)
			irgb [i] = Integer.parseInt(rgb[i]);

		MainTable.recentPublicationsRender = new GeneralRenderer(new Color(irgb[0], irgb[1], irgb[2]), Globals.prefs.getColor(JabRefPreferences.TABLE_TEXT));
